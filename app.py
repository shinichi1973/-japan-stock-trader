...
st.set_page_config(
    page_title="日本株 自動バックテスト Ver.3.3",
    page_icon="📈",
    layout="wide"
)

st.title("📈 日本株 自動バックテスト Ver.3.3")

st.caption(
    "過去5年の日足データを使って仮想売買を検証します。"
    "実際の注文は行いません。"
)
...
def comparison(df):
    ...
    for name, morning, ma, volume, price2000 in tests:
...
if start_button:
    ...
    if tr.empty:
        ...
        st.download_button(
            "⬇️ 売買履歴CSVを保存",
            data=csv,
            file_name="backtest_trades_ver3_3.csv",
            mime="text/csv"
        )
...
st.caption(
    "Ver.3.3 / 仮想売買専用。証券会社への実注文は行いません。"
)

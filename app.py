...
def code(x):
    return str(x).strip().upper().replace(".T","")

def ticker(x):
    return code(x)+".T"

def name(x):
    return STOCK_NAMES.get(code(x), code(x))
...
def stock_data(c, years):
    try:
        ...
        d=yf.download(ticker(c), start=start, end=end+timedelta(days=1),
        ...
        d["MA75_Slope"]=d.MA75-d.MA75.shift(5)
...
def market_data():
    try:
        ...
        o["MA25"]=c.rolling(25).mean(); o["MA75"]=c.rolling(75).mean()
...
def overseas_data():
    ...
    for k,s in syms.items():
        try:
            d=yf.download(s,start=start,end=end+timedelta(days=1),
                          auto_adjust=False,progress=False,threads=False)
            if d.empty: continue
            if isinstance(d.columns,pd.MultiIndex):d.columns=d.columns.get_level_values(0)
...
def tech_score(r,lo,hi):
    return float(
        18*(r.MA25>r.MA75)+18*(r.Close>r.MA200)+14*(r.Close>r.MA25)+
        14*(r.Volume>=r.VOL20)+14*(lo<=r.RSI<=hi)+
        10*(r.MA25_Slope>0)+6*(r.MA75_Slope>0)+
        3*(r.Ret5>0)+3*(r.Ret10>0)
    )
...
with st.sidebar:
    ...
    compare=st.checkbox("旧・売買代金TOP50を比較保存",True)
    universe=st.text_area("分析対象4桁コード（全銘柄を対象にする場合は全上場コードを貼付）",
                          DEFAULT,height=180)

codes=parse_codes(universe)
st.title("📈 日本株 AI投資アシスタント Ver.5.5 RC3.11")
st.caption(f"BUILD: {BUILD}")
st.caption("全銘柄母集団 → AI一次評価 → AIお勧めTOP50 → BUY / PRE-BUY / WATCH")
st.info("旧・約定/売買代金TOP50を入口から外し、AI総合評価を新しいスクリーニング入口にします。旧方式も比較用CSVとして残します。")

if not codes: st.error("銘柄コードを入力してください。"); st.stop()

with st.spinner(f"🧠 {len(codes)}銘柄を一次スクリーニング中…"):
    data={c:stock_data(c,years) for c in codes}
    data={c:d for c,d in data.items() if not d.empty}
...
for c,d in data.items():
    ...
    rows.append({
        "コード":c,"銘柄名":name(c),"株価":float(r.Close),
        "AI総合スコア":ai,"テクニカルスコア":tech,"急騰予兆スコア":surge,
        "急騰判定":surge_label(surge),"最終判定":final_label(ai,surge),
        "RSI":float(r.RSI),"5日騰落率":float(r.Ret5),"10日騰落率":float(r.Ret10),
...
all_ai["AI TOP50採用"]=all_ai["AI総合順位"]<=topn
ai_top50=all_ai.head(topn).copy()

old=all_ai.sort_values("平均売買代金",ascending=False).reset_index(drop=True)
old["旧方式順位"]=old.index+1
old50=old.head(50).copy()
a=set(ai_top50.コード); b=set(old50.コード)
both=a&b; ai_only=a-b; old_only=b-a
...
st.dataframe(ai_top50[["AI総合順位","コード","銘柄名","株価","AI総合スコア",
"テクニカルスコア","急騰予兆スコア","急騰判定","最終判定","RSI",
"5日騰落率","出来高倍率","25日線乖離率"]],use_container_width=True,hide_index=True)
...
if pool.empty: st.info("💤 今日は無理にBUYしません。")
    ...
    for i,(_,r) in enumerate(pool.head(3).iterrows()):
        medal=["🥇","🥈","🥉"][i]
        st.success(f"{medal} **{r['銘柄名']}（{r['コード']}）** AI {r['AI総合スコア']:.0f}点 ｜ {r['最終判定']} ｜ 急騰予兆 {r['急騰予兆スコア']:.0f}")

st.header("③ 🔵 急騰予兆")
surge=all_ai[(all_ai.急騰予兆スコア>=55)&(all_ai.急騰予兆スコア<85)].sort_values("急騰予兆スコア",ascending=False)
st.dataframe(surge[["コード","銘柄名","急騰予兆スコア","急騰判定","AI総合スコア",
"株価","RSI","5日騰落率","出来高倍率","25日線乖離率"]].head(20),
...
if compare:
    st.dataframe(old50[["旧方式順位","コード","銘柄名","株価","AI総合スコア",
                        "急騰予兆スコア","最終判定"]],use_container_width=True,hide_index=True)

# ---------------- CSV ----------------
all_ai["旧TOP50採用"]=all_ai.コード.isin(b)
all_ai["AIのみ"]=all_ai.コード.isin(ai_only)
all_ai["旧のみ"]=all_ai.コード.isin(old_only)
settings=pd.DataFrame({
"項目":["BUILD","未来情報","AI TOP50","旧TOP50","急騰予兆","2,000円以上BUY"],
"設定":[BUILD,"不使用",f"上位{topn}","平均売買代金上位50","当日までの価格・出来高・RSI等",
        "除外" if exclude2000 else "許可"]})
files={
"00_summary.csv":comparison,
"01_ai_top50.csv":ai_top50,
"02_all_ai_screening.csv":all_ai,
"03_old_turnover_top50.csv":old50,
"04_ai_only.csv":all_ai[all_ai["AIのみ"]],
"05_old_only.csv":all_ai[all_ai["旧のみ"]],
"06_overlap.csv":all_ai[all_ai.コード.isin(both)],
"07_surge_candidates.csv":surge,
"08_buy_top3.csv":pool.head(3),
"09_settings.csv":settings}
buf=BytesIO()
...
st.divider()
st.download_button("📦 RC3.11 全分析CSVをZIPでダウンロード",
                   data=buf.getvalue(),
                   file_name="ver5_5_RC3_11_AI_TOP50_analysis.zip",
                   mime="application/zip",
                   use_container_width=True)
st.caption("RC3.11はAI TOP50方式の検証版です。まず旧方式と並行比較し、実績で優位性を確認してから正式採用します。")
st.caption("※AIスコアは現段階ではルールベース。日々のCSVを蓄積して重み・閾値を検証します。")
